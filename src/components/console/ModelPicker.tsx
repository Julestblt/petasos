    ...
    <div className="flex flex-wrap items-center gap-2">
      <Combobox
        ...
        itemToStringLabel={(id) => {
          ...
          return `${model.model} ${model.provider_label} ${model.provider}`
        }}
        disabled={disabled || models.length === 0}
      >
        <div className="flex min-w-[14rem] max-w-[20rem] items-center gap-1.5">
          <Sparkles className="size-3.5 shrink-0 text-muted-foreground" />
          <ComboboxInput
            placeholder="Search models…"
            disabled={disabled || models.length === 0}
            className="h-8 min-w-0 flex-1 rounded-full bg-secondary/50"
            showClear
          />
        </div>
      ...
      {reasoningEnabled ? (
        <Select
          ...
          disabled={disabled}
        >
          <SelectTrigger size="sm" className="rounded-full bg-secondary/50">
