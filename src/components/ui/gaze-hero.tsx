...
export function GazeHero({
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'gaze-hero relative z-20 shrink-0 overflow-visible',
        className,
      )}
      style={
        {
          ...
          height: size,
          '--gaze-size': `${size}px`,
          '--gaze-eye': 'var(--primary-foreground)',
        } as CSSProperties
      }
    >
      {showGlow ? (
        <div
          ...
          style={{
            background:
              'radial-gradient(circle, color-mix(in oklch, var(--primary) 45%, transparent) 0%, color-mix(in oklch, var(--primary) 14%, transparent) 40%, transparent 68%)',
          }}
        />
      ) : null}
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(120% 120% at 72% 28%, color-mix(in oklch, var(--primary) 35%, white) 0%, var(--primary) 32%, color-mix(in oklch, var(--primary) 72%, black) 68%, color-mix(in oklch, var(--primary) 45%, black) 100%)',
          boxShadow: showGlow
            ? 'inset -18px -22px 36px color-mix(in oklch, black 28%, transparent), inset 14px 16px 28px color-mix(in oklch, white 22%, transparent), 0 0 40px color-mix(in oklch, var(--primary) 40%, transparent)'
            : 'inset -4px -5px 10px color-mix(in oklch, black 28%, transparent), inset 3px 4px 8px color-mix(in oklch, white 22%, transparent), 0 0 8px color-mix(in oklch, var(--primary) 40%, transparent)',
        }}
      >
        <div
          ref={faceRef}
        ...
        >
          <span
            className={cn(
              'gaze-eye block shrink-0 rounded-full',
              thinking && 'gaze-eye-thinking',
            )}
            style={{
              width: eyeWidth,
              height: eyeHeight,
              background: 'var(--gaze-eye, var(--foreground))',
            }}
          />
          <span
            className={cn(
      ...
      {thinking ? (
        <div className="gaze-fx pointer-events-none absolute inset-0 z-30 overflow-visible">
          ...
          <span className="gaze-spark gaze-spark-a absolute size-1.5 rounded-full bg-primary" />
          <span className="gaze-spark gaze-spark-b absolute size-1 rounded-full bg-primary/80" />
          <span className="gaze-spark gaze-spark-c absolute size-1 rounded-full bg-primary/70" />
        </div>
      ) : null}
    </div>
  )
